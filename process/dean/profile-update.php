<?php
    ...
    if($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["update-profile"])) {
        ...
        $id_number = htmlspecialchars(trim($_SESSION["dean-id"]));
        ...
        { 
            ...
            header("Location: ../../pages/deans/home.php?page=user-profile&update-profile=true");
            exit();
        }
            ...
            try {
                $update_credentials = $conn->prepare("UPDATE deans_credentials_tbl
                ...
                $update_info = $conn->prepare("UPDATE deans_info_tbl
        ...
        if(!in_array($file_extension, $allowed_img_format)) {
            ...
            header("Location: ../../pages/deans/home.php?page=user-profile&update-photo=true");
            exit();
        }
            ...
            try {
                ...
                $unique_filename = 'dean_' . $id_number . '_' . uniqid() . '.' . $file_extension;

                $update_profile_picture = $conn->prepare("UPDATE deans_info_tbl SET profile_picture = :profile_picture WHERE id_number = :id_number");
        ...
        if(empty($current_password) || empty($new_password) || empty($confirm_password)) {
            ...
            header("Location: ../../pages/deans/home.php?page=user-profile&update-password=true");
            exit();
        }
            ...
            if($new_password === $confirm_password) {
                try {
                    $check_account = $conn->prepare("SELECT * FROM deans_credentials_tbl WHERE id_number = :id_number LIMIT 1");
                    $check_account->execute([":id_number" => $id_number]);
                    
                    if($check_account->rowCount() === 1) {
                        $password_data = $check_account->fetch(PDO::FETCH_OBJ);

                        if(password_verify($current_password, $password_data->dean_password)) {
                            $set_new_password = $conn->prepare("UPDATE deans_credentials_tbl SET dean_password = :new_password, generated_password = :generated_password WHERE id_number = :id_number");
                            ...
                            $change_update = $conn->prepare("UPDATE deans_info_tbl
        ...
        try {
            ...
            if($check_account->rowCount() === 1) {
                $update_two_factor_authentication = $conn->prepare("UPDATE deans_credentials_tbl 
