<?php
    ...
    if($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["update-profile"])) {
        ...
        $id_number = htmlspecialchars(trim($_SESSION["observer-id"]));
        ...
        { 
            ...
            header("Location: ../../pages/observer/home.php?page=user-profile&update-profile=true");
            exit();
        }
            ...
            try {
                $update_credentials = $conn->prepare("UPDATE observers_credentials_tbl
                ...
                $update_info = $conn->prepare("UPDATE observers_info_tbl
        ...
        if(!in_array($file_extension, $allowed_img_format)) {
            ...
            header("Location: ../../pages/observer/home.php?page=user-profile&update-photo=true");
            exit();
        }
            ...
            try {
                ...
                $unique_filename = 'observer_' . $id_number . '_' . uniqid() . '.' . $file_extension;

                $update_profile_picture = $conn->prepare("UPDATE observers_info_tbl SET profile_picture = :profile_picture WHERE id_number = :id_number");
        ...
        if(empty($current_password) || empty($new_password) || empty($confirm_password)) {
            ...
            header("Location: ../../pages/observer/home.php?page=user-profile&update-password=true");
            exit();
        }
            ...
            if($new_password === $confirm_password) {
                try {
                    ...
                    if($check_account->rowCount() === 1) {
                        ...
                        if(password_verify($current_password, $password_data->observer_password)) {
                            $set_new_password = $conn->prepare("UPDATE observers_credentials_tbl SET observer_password = :new_password, generated_password = :generated_password WHERE id_number = :id_number");
                            ...
                            $change_update = $conn->prepare("UPDATE observers_info_tbl
        ...
        try {
            ...
            if($check_account->rowCount() === 1) {
                $update_two_factor_authentication = $conn->prepare("UPDATE observers_credentials_tbl 
