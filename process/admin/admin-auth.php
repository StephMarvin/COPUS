<?php
    ...
    if($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["admin-login"])) {
        ...
        if(empty($id_number) || empty($password)) {
            ...
            try {
                $check_admin_id = $conn->prepare("SELECT * FROM admin_credentials_tbl WHERE id_number = :id_number AND is_archived = 'No' LIMIT 1");
                $check_admin_id->execute([":id_number" => $id_number]);

                if($check_admin_id->rowCount() === 1) {
                    $admin_credentials = $check_admin_id->fetch(PDO::FETCH_OBJ);

                    if(password_verify($password, $admin_credentials->admin_password)) {

                        if($admin_credentials->locked_account === "Yes") {
                            $_SESSION["locked-account"] = true;
                            header("Location: ../../pages/locked/locked-account.php");
                            exit();
                        }

                        else if($admin_credentials->two_factor_authentication === "Disabled") {
                            
                            $update_login = $conn->prepare("UPDATE admin_credentials_tbl SET last_login = CURRENT_TIMESTAMP() WHERE id_number = :id_number");
                            $update_login->execute([":id_number" => $id_number]);
                            
                            $_SESSION["admin-id"] = $id_number; 
                            ...
                            $full_name = $admin_credentials->first_name . " " . $admin_credentials->last_name;
                            $email_address = $admin_credentials->email_address;

                            $otp_code = generate_otp_code();
                            $otp_expiry = generate_expiry_time(5);

                            $update_otp = $conn->prepare("UPDATE admin_credentials_tbl SET otp_code = :otp_code, otp_code_expiry = :otp_expiry WHERE id_number = :id_number");
                            $update_otp->execute([
                                ":otp_code" => $otp_code,
                            ...
                            $_SESSION["temp-login"] = ["admin-id" => $id_number, "otp-expiry" => $otp_expiry];
                            ...
                            header("Location: ../../pages/admin/auth.php");
                            exit();
                        }
                    }
        ...
        $admin_id = htmlspecialchars(trim($_SESSION["temp-login"]["admin-id"]));
        ...
        if(empty($otp_code)) {
            ...
            try {
                $check_otp = $conn->prepare("SELECT * FROM admin_credentials_tbl WHERE id_number = :id_number AND otp_code = :otp LIMIT 1");
                $check_otp->execute([
                    ":id_number" => $admin_id,
                    ":otp" => $otp_code
                ]);
                ...
                if($check_otp->rowCount() === 1) {
                    $admin_credentials = $check_otp->fetch(PDO::FETCH_OBJ);

                    if(strtotime($admin_credentials->otp_code_expiry) >= time()) {

                        $update_login = $conn->prepare("UPDATE admin_credentials_tbl SET last_login = CURRENT_TIMESTAMP() WHERE id_number = :id_number");
                        $update_login->execute([":id_number"  => $admin_id]);
    ...
    // Reset Admin Password
        ...
        if(empty($email_address) || empty($id_number)) {
            ...
            header("Location: ../../pages/admin/forgot-password.php");
            exit();
        }
            ...
            if(filter_var($email_address, FILTER_VALIDATE_EMAIL)) {
                try {
                    $check_credentials = $conn->prepare("SELECT * FROM admin_credentials_tbl WHERE id_number = :id_number AND email_address = :email_address LIMIT 1");
                    $check_credentials->execute([":id_number" => $id_number, ":email_address" => $email_address]);

                    if($check_credentials->rowCount() === 1) {             
                        $admin_details = $check_credentials->fetch(PDO::FETCH_OBJ);
                        $full_name = $admin_details->first_name . " " . $admin_details->last_name;

                        $reset_token = generate_reset_token();
                        $token_expiry = generate_expiry_time(5);

                        $update_reset_token = $conn->prepare("UPDATE admin_credentials_tbl SET password_reset_token = :token, reset_token_expiry = :expiry WHERE id_number = :id_number");
                        ...
                        $_SESSION["temp-reset"] = ["admin-id" => $id_number, "token-expiry" => $token_expiry];
                        ...
                        header("Location: ../../pages/admin/reset-password.php");
                        exit();
                    }
        ...
        $id_number = $_SESSION["temp-reset"]["admin-id"];
        ...
        if(empty($id_number) || empty($reset_token) || empty($new_password) || empty($confirm_password)) {
            ...
            if($new_password === $confirm_password) {
                try {
                    $check_token = $conn->prepare("SELECT * FROM admin_credentials_tbl WHERE password_reset_token = :token AND id_number = :id_number LIMIT 1");
                    $check_token->execute([
                        ":token" => $reset_token,
                        ":id_number" => $id_number
                    ]);
                    ...
                    if($check_token->rowCount() === 1) {
                        ...
                        if(strtotime($token_details->reset_token_expiry) >= time()) {
                            $update_password = $conn->prepare("UPDATE admin_credentials_tbl SET admin_password = :admin_password, generated_password = :generated_password WHERE id_number = :id_number");
                            $update_password->execute([
                                ":admin_password" => password_hash($new_password, PASSWORD_BCRYPT),
